...
group.add_argument('--spacegroup', type=int, nargs='+', help='The space group id to be sampled (1-230)')
...
if args.optimizer != "none":
    ...
    midfix = "-".join([str(spg) for spg in args.spacegroup])
    filename = os.path.join(output_path, f"{name}_{midfix}.{extension}")
    # loop over different spacegroup
    for spg in args.spacegroup:
        for batch_idx in range(num_batches):
            ...
            XYZ, A, W, M, L = sample_crystal(
                ...
                args.n_max, n_sample, args.atom_types, args.wyck_types, args.Kx, args.Kl,
                spg, w_mask, atom_mask, args.top_p, args.temperature, T1, constraints
            )

            G = spg * jnp.ones((n_sample), dtype=int)
            if args.mcmc:
                x = (G, L, XYZ, A, W)
                key, subkey = jax.random.split(key)
