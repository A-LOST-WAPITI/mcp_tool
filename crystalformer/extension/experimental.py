...
def make_multi_cond_logp(logp_fn, forward_fns, targets, alphas):
    ...
    def make_callback_forward(forward_fn):
        def callback_forward(G, L, XYZ, A, W, target):
            result_shape = jax.ShapeDtypeStruct(G.shape, jnp.float32)
            return jax.experimental.io_callback(forward_fn, result_shape, G, L, XYZ, A, W, target)
        return callback_forward

    forward_fns = [make_callback_forward(forward_fn) for forward_fn in forward_fns]
    ...
    def cond_logp_fn(params, key, G, L, XYZ, A, W, is_training):
        ...
        # calculate multiple p(y|x) and sum them up
        logp_cond = 0
        for i in range(num_conditions):
            _logp_cond = forward_fns[i](G, L, XYZ, A, W, targets[i])
            logp_cond += _logp_cond.squeeze() * alphas[i]
